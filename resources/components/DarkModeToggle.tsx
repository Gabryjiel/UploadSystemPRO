...
import { Toggle } from '../typings'

export const DarkModeToggle = (props: Toggle) => {
  const { width, height, size, enabled } = props
  const sizing = size ? { width: size, height: size } : { width, height }

  return (
    <svg {...sizing} stroke='currentColor' fill='currentColor' strokeWidth='0' viewBox='0 0 24 24' xmlns='http://www.w3.org/2000/svg'>
      {enabled ?
