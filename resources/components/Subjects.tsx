...
import { TSubjectProps } from '../typings'
import { IconPlus } from '../icons'

export const Subjects = () => {
  const role = useContext(RoleContext)
  const [subjects, setSubjects] = useState<TSubjectProps[] | null | undefined>(null)
  const [search, setSearch] = useState<TSubjectProps[] | null>(null)
  const [query, setQuery] = useState<string>('')

  useEffect(() => {
    request<TSubjectProps[]>('subjects').then(Object.values).then((data) => subjects === null && setSubjects(data))
  ...
  useEffect(() => {
    ...
    const search = subjects ? subjects.filter(({ name, group, subgroup, semester }) => {
      return includes(name, query) || includes(group, query) || includes(subgroup, query) || includes(semester, query)
    }) : null

    setSearch(search)
  }, [query])

  const SubjectTable = (props: { content?: TSubjectProps[] | null }): JSX.Element => {
    return (
      <div className='grid grid-cols-subjects items-center gap-2 font-medium dark:font-normal'>
        {props.content?.map(({ id, description, name, students, group, subgroup, semester, assignments, not_graded, not_answered }) => (
          <Fragment key={id}>
            ...
            <div className='text-center text-xs sm:text-base whitespace-nowrap'>
              ...
              <span>{not_answered || not_graded} new</span>
            </div>
