  ...
  const SubjectTable = (props: { content: TSubject[] | null }): JSX.Element => {
    return (
      <div className='grid grid-cols-subjects items-center gap-2 font-medium dark:font-normal'>
        {props.content?.map(({ id, code, name }) => (
          <Fragment key={id}>
  ...
  return (
    <div className='stack'>
      <div className='hstack mb-5 justify-between'>
        ...
        <Link className='hidden sm:block self-center border-current border-1 px-3 py-1 cursor-pointer hover:text-white hover:bg-black dark:hover:text-black dark:hover:bg-gray-200' to='/classes/new'>new</Link>
      </div>
